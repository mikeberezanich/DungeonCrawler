...
public class Weapon extends Item {
	...
	public Weapon(int floorLevel, int[] coordinates) {
		...
		if (classification == SWORD_CLASS){
			strength = itemRng.nextInt(floorLevel + 1) + 5; //Fix this up later
			weight = 2;
			...
			strength = itemRng.nextInt(floorLevel + 1) + 3; //Fix this up later
			weight = 1;
			...
			strength = itemRng.nextInt(floorLevel + 1) + 7; //Fix this up later
			weight = 3;
